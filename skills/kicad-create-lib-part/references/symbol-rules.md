...
> 但那是 KiCad 的风格，不是我们的。我们跟自己的库对齐。

代价要认：49 个引脚的器件按 200 mil 排，本体 **55.9 × 121.9 mm**
...
顶对齐时第一个引脚的 y = 最大跨度 / 2。若把 `pitch_mil` 设成 150、
`group_gap_mil` 设成 300：
...
但最省事的办法是就让 `pitch_mil` / `group_gap_mil` 都是 100 的整数倍
（默认的 200 / 400 满足）。
